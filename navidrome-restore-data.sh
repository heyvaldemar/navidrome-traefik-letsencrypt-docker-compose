#!/usr/bin/env bash
# navidrome-restore-data.sh [backup-file-name]
#
# Replaces Navidrome's application data with one of the archives the backups
# service wrote.
#
#   ./navidrome-restore-data.sh               list and ask
#   ./navidrome-restore-data.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than navidrome.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-navidrome}"
APP_SERVICE="navidrome"
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using Navidrome." >&2
  exit 1
fi
