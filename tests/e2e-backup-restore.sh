#!/bin/bash
# End-to-end tests for the navidrome-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# This application keeps everything it knows in one directory: a SQLite file
# holding users, playlists, play counts and ratings, beside the search index
# and the artwork cache. There is no database server to dump, so the archive IS
# the database, and the test that matters is whether a file written before a
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-navidrome}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-navidrome-traefik-letsencrypt-docker-compose.yml}"
...
: "${DATA_BACKUPS_PATH:=/srv/navidrome-data/backups}"
: "${DATA_BACKUP_NAME:=navidrome-data-backup}"
: "${DATA_PATH:=/data}"
: "${BACKUP_INTERVAL:=24h}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq navidrome | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: navidrome container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^NAVIDROME_HOSTNAME=.' .env || { echo "  NAVIDROME_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_database() {
  ...
  # navidrome.db is the whole point of taking it: users, playlists, play counts
  # and ratings. Everything else in there is a cache that rebuilds itself, so
  # an archive missing this one file restores into an instance that starts
  # cleanly and has forgotten every listener.
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^data/navidrome.db'" \
    || { echo "  $f does not carry navidrome.db" >&2; return 1; }
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./navidrome-restore-data.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./navidrome-restore-data.sh failed" >&2; return 1; }
